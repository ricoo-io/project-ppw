<?php
    ...
    $warrow = $db->getALL("SELECT * FROM t_colors ORDER BY f_colour ASC");

    $selectedColors = $db->getALL("SELECT f_idwarna FROM barang_color WHERE f_idbarang = $id");
    $selectedColorIds = is_array($selectedColors) ? array_column($selectedColors, 'f_idwarna') : [];

    $selectedSizes = $db->getALL("SELECT f_idukuran FROM barang_ukuran WHERE f_idbarang = $id");
    $selectedSizeIds = is_array($selectedSizes) ? array_column($selectedSizes, 'f_idukuran') : [];
...
if (isset($_POST['simpan'])) {
    ...
    // Update barang
    $sql = "UPDATE t_barang SET f_idkategori=$idkat, f_pakaian='$bar', f_gambar='$gam', f_harga=$har, 
    f_quantity=$qu, f_rating= $ra, f_detail='$desk' WHERE f_id='$id'";
    $db->runSQL($sql);

    // Hapus warna lama di tabel barang_color
    $db->runSQL("DELETE FROM barang_color WHERE f_idbarang = $id");

    // Tambahkan warna baru ke tabel barang_color
    if (!empty($colors)) {
        foreach ($colors as $colorId) {
            $db->runSQL("INSERT INTO barang_color (f_idbarang, f_idwarna) VALUES ($id, $colorId)");
        }
    }

    $db->runSQL("DELETE FROM barang_ukuran WHERE f_idbarang = $id");

    // Tambahkan ukuran baru ke tabel barang_ukuran
    if (!empty($ukur)) {
        foreach ($ukur as $ukuranId) {
            $db->runSQL("INSERT INTO barang_ukuran (f_idbarang, f_idukuran) VALUES ($id, $ukuranId)");
        }
    }
                            ...
                            <form action="" method="post" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-input" name="ukuran[]" id="" multiple>
                                        <?php foreach ($ukrow as $u) : ?>
                                            <option value="<?php echo $u['f_id'] ?>" 
                                                <?php if (in_array($u['f_id'], $selectedSizeIds)) {
                                                    echo "selected";
                                                } ?>>
                                                <?php echo $u['f_ukuran'] ?>
                                            </option>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <select class="form-input" name="warna[]" id="" multiple>
                                        <?php foreach ($warrow as $w) : ?>
                                            <option value="<?php echo $w['f_id'] ?>" 
                                                <?php if (in_array($w['f_id'], $selectedColorIds)) {
                                                    echo "selected";
                                                } ?>>
                                                <?php echo $w['f_colour'] ?>
                                            </option>
                                ...
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="number" id="rate" name="rating" required value="<?php echo $item['f_rating'] ?>" class="form-input">
                                </div>
