<?php
    ...
    $sql = "SELECT DISTINCT t_barang.f_id,t_kategori.f_kategori,t_ukuran.f_ukuran, t_barang.f_pakaian, t_barang.f_gambar ,t_barang.f_harga, t_barang.f_quantity, t_barang.f_detail FROM t_barang 
    INNER JOIN t_kategori ON t_barang.f_idkategori = t_kategori.f_id 
    INNER JOIN t_ukuran ON t_barang.f_idukuran = t_ukuran.f_id ORDER BY f_id DESC LIMIT $mulai, $banyak";
    $row = $db->getALL($sql);
    $no = 1 + $mulai;
?>
<!DOCTYPE html>
                ...
                <?php if(!empty($row)) { ?>
                    <?php foreach ($row as $r) : ?>
                        <tr>
                            ...
                            <td><?php echo $r['f_ukuran'] ?></td>
                            <td><?php echo $r['f_pakaian'] ?></td>
                            <td><img style="width:85px" src="../public/images/<?php echo $r['f_gambar'] ?>" alt=""></td>
                            <td><?php echo $r['f_harga'] ?></td>
                            <td><?php echo $r['f_quantity'] ?></td>
                            <td><?php echo $r['f_detail'] ?></td>
